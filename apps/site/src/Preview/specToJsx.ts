...
import { normalizeSpec, type NormalizedRow } from "./SpecScreen";
...
export function specToJsx(spec: ScreenSpec): string {
  ...
  for (const group of s.groups) {
    ...
    body.push(element("List", [attr("header", group.header), attr("footer", group.footer)], rows, I));
  }
  ...
  if (s.tabs.length > 0) {
    ...
    body.push(element("TabBar", [attr("defaultValue", s.tabs[0].value)], items, I));
  }
...
// Layout: nav bar (+ search) pinned on top, lists and buttons in a scrolling
// pinstripe body, tab bar pinned to the bottom.
